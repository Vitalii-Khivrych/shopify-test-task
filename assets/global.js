...
function initVariantImageFiltering() {
  ...
  const section = variantSelects.closest("section");
  if (!section) return;
  ...
  const filterImages = (currentColor) => {
    const allMediaItems = document.querySelectorAll("slider-component img");

    allMediaItems.forEach((item) => {
      const mediaAlt = item?.alt.toLowerCase();
      const parentEl = item.closest("li");

      if (mediaAlt === currentColor) {
        if (!parentEl) return;
  ...
  section?.addEventListener("change", (e) => {
    if (e.target.matches('variant-selects input[type="radio"]')) {
      filterImages(e.target.value.toLowerCase());
    }
  });

  filterImages(getCheckedValue().toLowerCase());
}

document.addEventListener("DOMContentLoaded", initVariantImageFiltering);
